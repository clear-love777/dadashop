...
class SPU(models.Model):
    ...
    #进了公司 先舔 舔数据库+models
    #on_delete 级联删除 PROTECT 删除主表数据时,从表如果主表关联的数据,则不让直接删除主表数据[开发时,尽量统一规范]
    #releted_name 指定反向查询的属性名,原反向属性名->类名_set
...
class SKU(models.Model):
    ...
    #该字段将图片存储至 指定文件夹->settings.MEDIA_ROOT+upload_to参数的值 ,且该字段对应的值为图片在服务器上的存储路径 var/a.jap
    #BASE_DIR+media/sku_default/
    default_image_url = models.ImageField(upload_to="sku_default/",
                                          verbose_name='默认图片', default=None)
